...
→ `python3` → `python` — requiring an importable `localcider` at each
candidate. This wheel-bundled copy deliberately omits the checkout's
repo-relative `.venv/bin/python` / `.pixi/envs/default/bin/python` candidates:
`pkg_dir/../..` here is `src/phasepred/data`, not the checkout, so those
candidates would point at directories that do not exist (D40, Ruling 2).
