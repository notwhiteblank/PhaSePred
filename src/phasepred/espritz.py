...
from phasepred.tool_paths import contract_env

ESPRITZ_RUNNER = Path(__file__).resolve().parents[2] / "tools" / "ESpritz" / "run"
...
class ESpritzFeature:
    ...
    @classmethod
    def from_json(cls, payload: dict[str, Any]) -> ESpritzFeature:
        return cls(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="phasepred_espritz_") as tmp:
        ...
        subprocess.run(
            ...
            check=True,
            cwd=str(ESPRITZ_RUNNER.parents[2]),
