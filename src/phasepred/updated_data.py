...
def build_task_split(
    ...
    # --- Positives ---
    subset = label_table[label_table["is_human"]] if human_only else label_table
    positives = subset[subset[positive_col]]["UniprotEntry"].unique().tolist()

    # --- Negative pool ---
    # 1. PhaSepDB3 proteins that are not positive for this task
    phasepdb_neg = subset[
        (subset["source"] == "phasepdb3") & ~subset[positive_col]
    ]["UniprotEntry"].unique().tolist()

    # 2. LLPSDB2 confirmed negatives
    llps_neg = subset[subset["is_ps_negative"]]["UniprotEntry"].unique().tolist()
...
def _load_background_negatives(
    ...
    for col in ["is_ps_self", "is_ps_other"]:
        if col in label_table.columns:
            all_positive_ids.update(label_table[label_table[col]]["UniprotEntry"].tolist())
    # Also exclude LLPSDB2 confirmed positives
    if "is_any_ps" in label_table.columns:
        all_positive_ids.update(label_table[label_table["is_any_ps"]]["UniprotEntry"].tolist())
