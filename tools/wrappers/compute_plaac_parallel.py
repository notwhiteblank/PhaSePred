#!/usr/bin/env python
"""Run PLAAC in parallel across N chunks of a sequence table."""
...
PLAAC_RUNNER = REPO_ROOT / "scripts" / "tools" / "run_plaac.sh"


def run_plaac_chunk(args: tuple[int, list[dict]]) -> tuple[int, str]:
    chunk_idx, records = args
    fasta_text = "".join(
        f">{r['UniprotEntry']}\n{str(r['sequence']).upper()}\n" for r in records
    )
    with tempfile.TemporaryDirectory(prefix=f"plaac_chunk{chunk_idx}_") as tmp:
...
def parse_plaac(text: str) -> list[dict[str, str | float]]:
    ...
    for line in lines:
        ...
        if "SEQid" in record and "NLLR" in record:
            rows.append({"UniprotEntry": record["SEQid"], "PLAAC": float(record["NLLR"])})
    return rows
...
def main() -> None:
    ...
    print(f"Running PLAAC over {len(records)} sequences in {len(chunks)} chunks...")
    all_rows = []
    with Pool(processes=min(len(chunks), n_jobs)) as pool:
        for chunk_idx, output_text in pool.imap_unordered(run_plaac_chunk, chunks):
            rows = parse_plaac(output_text)
    ...
    print(f"Wrote {len(result)} PLAAC rows to {output_path}")
